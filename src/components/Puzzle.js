...
function Puzzle() {
	...
	const puzzleess = async () => {
		...
		if (Array.isArray(response.data.images)) {
			...
			const shuffledImages = [...response.data.images].sort(
				() => Math.random() - 0.5
			);
			...
			{dataImages ? (
				modalShow ? (
					<div className="tebrik bg-success text-light text-center mt-5 mx-5 p-1 ">
						...
						<hr />
						<Container
							className="tebrik-div mt-5"
							style={{ display: 'flex', justifyContent: 'space-between' }}
						>
							<div style={{ flex: 1, marginRight: '20px' }}>
								Tamamlama Süren: {displayTime()}
							</div>
							<div style={{ flex: 1, marginRight: '20px' }}>
								Hamle Sayın: {numberOfMoves}
							</div>
							<div style={{ flex: 1, marginRight: '20px' }}>
								Puanın : {point}
							</div>
